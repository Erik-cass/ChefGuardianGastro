    ...
    if ($count == 1) {
        echo "<script>
         alert('Ese email ya está en nuestra base de datos.');
          window.location.href = './views/registeralumadmi.php';  //Redirigir al usuario a register.php despues de cerrar la alerta 
        </script>";
    } else {
          ...
          if (mysqli_query($conn, $query)) {
            echo "<script>
              alert('!La cuenta ha sido creada!');
                window.location.href = './views/registeralumenca.php';
                </script>";
          } else {
