...
class PDF extends FPDF {
    function Header() {
        ...
        $this->Cell(0, 6, utf8_decode('Lista de Artículos Perdidos'), 0, 1, 'C');
    ...
    function ImprovedTable($header, $data) {
        ...
        foreach ($data as $row) {
            ...
            $this->Cell($w[6], 15, utf8_decode($row['estado']), 1);
            $this->Ln();
        }
    }
}
...
$query = "SELECT id_articulo, nombre, categoria, descripcion, cantidad, fh, estado FROM articulos_perdidos";
$result = $conn->query($query);

$header = array('ID', 'Nombre', 'Categoría', 'Descripción', 'Cantidad', 'Fecha', 'Estado');
$data = array();
...
if ($result->num_rows > 0) {
    ...
    $data[] = array('id_articulo' => 'No disponible', 'nombre' => 'No disponible', 'categoria' => 'No disponible', 'descripcion' => 'No disponible', 'cantidad' => 'No disponible', 'fh' => 'No disponible', 'estado' => 'No disponible');
}
